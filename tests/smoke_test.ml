...
let suite = 
  ...
  "smoke test" >:::
    ...
    (*
      assert_equal ( Redis.sadd "tim" "smells" conn);
    *)
    (*

      assert_equal ( Redis.srem "tim" "smells" conn);
      
      assert_equal ( "not cool" = Redis.string_of_bulk_data (Redis.spop "tim" conn) );
      
      ignore ( Redis.del_one "rory" conn);
      assert_equal ( Redis.sadd "rory" "cool" conn);
      assert_equal ( Redis.sadd "tim" "even cooler" conn);
    *)
    (*
    assert_equal ( Redis.smove "tim" "rory" "even cooler" conn );
    
    assert_equal ( 2 = Redis.scard "rory" conn );

    assert_equal ( Redis.sismember "rory" "cool" conn );
...
(*
    ...
    Redis.select 0 conn;
    assert_equal ( Redis.move "rory" 1 conn );
    ...
    (* Sorted sets *)
    assert_equal (Redis.zadd "coolest" 42.0 "rory" conn);
    assert_equal (Redis.zrem "coolest" "rory" conn);
...
(*
    ...
    assert_equal (2 = Redis.zunionstore "union" ["rory"; "tim"] ~aggregate:`Sum conn);
    assert_equal (2 = Redis.zunionstore "union" ["rory"; "tim"] ~aggregate:`Min conn);
    assert_equal (2 = Redis.zunionstore "union" ["rory"; "tim"] ~aggregate:`Max conn);

    assert_equal (2 = Redis.zunionstore_withweights "union" ["rory"; "tim"] [1.0; 0.5] conn);
    assert_equal (2 = Redis.zunionstore_withweights "union" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Sum conn);
    assert_equal (2 = Redis.zunionstore_withweights "union" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Min conn);
    assert_equal (2 = Redis.zunionstore_withweights "union" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Max conn);

    assert_equal (0 = Redis.zinterstore "inter" ["rory"; "tim"] conn);
    assert_equal (0 = Redis.zinterstore "inter" ["rory"; "tim"] ~aggregate:`Sum conn);
    assert_equal (0 = Redis.zinterstore "inter" ["rory"; "tim"] ~aggregate:`Min conn);
    assert_equal (0 = Redis.zinterstore "inter" ["rory"; "tim"] ~aggregate:`Max conn);
*)
(*

    assert_equal (0 = Redis.zinterstore_withweights "inter" ["rory"; "tim"] [1.0; 0.5] conn);
    assert_equal (0 = Redis.zinterstore_withweights "inter" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Sum conn);
    assert_equal (0 = Redis.zinterstore_withweights "inter" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Min conn);
    assert_equal (0 = Redis.zinterstore_withweights "inter" ["rory"; "tim"] [1.0; 0.5] ~aggregate:`Max conn);
    ...
    ignore (Redis.lpush "rory" "1" conn);
    ignore (Redis.lpush "rory" "2" conn);
    assert_equal ( "2" = Redis.string_of_bulk_data (List.hd (
    ...
    (* Hashes *)
    ignore (Redis.del ["rory"] conn);
    assert_equal (Redis.hset "rory" "cool" "true" conn);
    assert_equal (not (Redis.hset "rory" "cool" "false" conn));

    assert_equal (Redis.hdel "rory" "cool" conn);
    assert_equal (not (Redis.hdel "rory" "cool" conn));

    assert_equal (Redis.hset "rory" "handsome" "true" conn);
    assert_equal (Redis.String("true") = Redis.hget "rory" "handsome" conn);
    assert_equal (Redis.Nil = Redis.hget "rory" "boring" conn);

    assert_equal ([Redis.String("true"); Redis.Nil] = Redis.hmget "rory" ["handsome"; "boring"] conn);

    Redis.hmset "rory" [("handsome", "false"); ("boring", "true")] conn;

    assert_equal(26 = Redis.hincrby "rory" "age" 26 conn);

    assert_equal(Redis.hexists "rory" "handsome" conn);
    assert_equal(not (Redis.hexists "rory" "andsome" conn));
    ...
    assert_equal ( "master" = Redis.Info.get
        (Redis.info conn)
        "role"
    );
    ...
    ignore (Redis.rpush "rory" "cool" conn);
    assert_equal ((Redis.string_of_bulk_data (Redis.blpop "rory" conn)) = "cool");
    assert_equal ((Redis.blpop "rory" ~timeout:(`Seconds(1)) conn) = Redis.Nil);

    ignore (Redis.rpush "tim" "not cool" conn);
    assert_equal ((Redis.blpop_many ["rory"; "tim"] conn) = ("tim", Redis.String("not cool")));
    assert_equal ((Redis.blpop_many ["rory"; "tim"] ~timeout:(`Seconds(1)) conn) = ("", Redis.Nil));

    ignore (Redis.rpush "rory" "cool" conn);
    assert_equal ((Redis.string_of_bulk_data (Redis.brpop "rory" conn)) = "cool");
    assert_equal ((Redis.brpop "rory" ~timeout:(`Seconds(1)) conn) = Redis.Nil);

    ignore (Redis.rpush "tim" "not cool" conn);
    assert_equal ((Redis.blpop_many ["rory"; "tim"; "bob"] conn) = ("tim", Redis.String("not cool")));
    assert_equal ((Redis.blpop_many ["rory"; "tim"; "bob"] ~timeout:(`Seconds(1)) conn) = ("", Redis.Nil));
    ...
    assert_equal ( 0.0 < Redis.lastsave conn);
